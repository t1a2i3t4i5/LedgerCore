...
issue を 1 本の PR にします。**実装 → テスト → コミット → PR 作成までで終わります** — レビューもマージもしません。作成した PR は変更リスクを判定してから、対応するレビューへ渡します。
...
| 2   | **着手可否を判定する** — 「完了条件」と、明記された「対象外」を作業リストに写し、仕様の穴だけを聞く | （なし。穴は質問で埋める）                                         |
...
「完了条件」をそのまま作業リストに写します。「対象外」が明記されている場合だけ、変更しない範囲として同じ作業リストに加えます。**完了条件が完了判定、明記された対象外が変更範囲の制約です。**
...
| --- | --- | --- |
| 完了の判定 | 完了条件 | `受け入れ条件` / `合格条件`（#54） |
| やらないこと | 対象外（任意） | `含めないもの` |

「対象外」は任意です。節が無いことを理由に、変更範囲を issue に書かれたファイルだけへ狭めたり、質問したりしません。

完了条件がどうしても無いときだけ、issue 本文から**こちらで起草し、質問バッチの 1 問目で確認します**。判定できないまま実装しません。

**「完了条件が無い＝穴ゼロ」と評価して、黙って実装を始めないこと。**
...
1. 「完了条件」のどれかが、実装しても ○× を判定できない
2. 明記された「対象外」に含まれる変更が必要になった（範囲が変わる）
...
論理単位で小さく分けます。メッセージは日本語で、末尾に実行中のエージェントで規定された `Co-Authored-By` trailer を付けます。
...
`--body-file` で組み立てます。`.github/` に PR テンプレートは無く、非対話ではテンプレートが発火しないためです。**`--draft` を付けません** — 作成直後にリスク段階に応じたレビューへ渡せる状態にします。
...
**`Closes` を書かない例外**: issue に「マージ後に判定する」完了条件があるときです。書くとマージした瞬間に自動クローズされ、確認前に閉じます。代わりに `#<番号>` と参照だけ書き、確認後に手動でクローズします。

手順 2・4 で既定値として決めたことがあれば、「判断したこと」として本文に列挙します。

### 報告して終わる

PR 番号と URL、そして `docs/git-workflow.md` のリスク判定表を読んで決めた段階を報告します。軽微ならレビュー不要、通常なら `/quick-review <PR番号>`、高リスクなら `/independent-review <PR番号>` を次の手順として案内します。判定条件はここに書き写しません。
