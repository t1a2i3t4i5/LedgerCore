...
`formatYen()` と同じファイルに置いてある。金額の整形ではないが、集約する理由は同じ形をしている — 構成比は**円グラフの扇形ラベルと集計画面のカテゴリ別リストという別々の場所に、同じ値として出る**。`toStringAsFixed` を各所で組み立てると、片方の桁数だけ変えたときに同じ画面の中でグラフとリストの数字が食い違い、しかも変えた本人には一方しか見えない。

**丸めて合計 100% にならないのは仕様。** 3 等分なら 33.3% × 3 = 99.9% になるが、補正する処理は入れない。扇形ラベルとリストが同じ関数を通っている以上、両者の見え方は揃う。
...
`ListTile` はこの潰れを overflow の例外にせず静かに畳むうえ、`find.text()` は畳まれた `Text` にもマッチするので、テストも黙って通ってしまう。省略が起きたかは `RenderParagraph.didExceedMaxLines` で見る（`test/widgets/summary_screen_chart_test.dart` の `_isEllipsized`）。
