...
そのため「メンバー 0 件・取引 0 件」は壊れた状態ではなく、正常な新規 DB の印になる。
ここに既定メンバーを 1 件でも戻すと初期設定が二度と出なくなるので、初期データを増やさない。
メンバー 0 件なのに取引が残っている組み合わせだけはデータ不整合として扱い、
起動時にエラーを出す（`StartupProvider`）。メンバーは2人までで、
`AppDatabase.insertMember()` のトランザクション内で既存件数を確認してから追加する。
画面側も2人に達したら追加導線を隠し、理由を表示する。行数を参照する制約は通常の
`CHECK` では表せないため、SQLiteのトリガーやスキーマ制約は追加しない。
3人以上の既存DBは存在しない前提とし、データを削除する移行や互換計算、専用案内は
追加しない。万一2人以外の状態なら、データは変更せず精算だけを算出しない。
