...
実体は `.claude/skills/review3/SKILL.md` に置いたプロジェクトスキルです（かつては `.claude/commands/review3.md` にあり、Claude Code のカスタムコマンドはスキルに統合されました）。

運用上の要点は次のとおりです。手順の詳細はこの `SKILL.md` が唯一の情報源です。
...
- **重大度はレビュアーが付けません。** 3 体の基準がずれると集約側で付け直すことになり、その順序がまた新しい食い違いを生むためです。重大度表は `SKILL.md` にだけあります
