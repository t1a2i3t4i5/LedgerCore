...
- **2 回目以降は、既に issue になっている指摘を「既知」に仕分けます。** 別 issue に切り出した指摘は PR 内のコードが変わらないので、同じ PR を再レビューすると必ず再掲されます(実際 issue #30 は PR #27 と #28 の両方で挙がりました)。3 体の報告が出揃った**あと**に限り、集約側が open issue と突き合わせて既知のものを本編から外し、実測検証も省きます。**PR のコメントは引き続き読みません** — 読むと 3 体の報告の答え合わせに使ってしまい、独立性が消えるためです
