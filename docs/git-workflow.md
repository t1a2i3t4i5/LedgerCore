...
- **3 体を並列で回すぶん使用量は大きいので、消費の内訳を実測して抑えてあります。** 過去 8 回分を計測したところ、1 回のレビューにつき 3 体合計で cache 読み込みが 404 万トークン、うち 89% が「1 ターンごとにそれまでの全コンテキストを読み直す」分でした。出力は全体の 1.4% しかなく、**効くのはターン数**です。そこで(1)各レビュアーの定義に探索の規律(`Bash` は差分取得だけ・`Read` は範囲を絞る・`Read`＋`Grep` は 12 回まで)を持たせ、(2)`reviewer-design` と `reviewer-tests-docs` を `sonnet` + `effort: medium` に下げました。**`reviewer-correctness` だけは `opus` + `effort: high` のまま**です — `Infinity` の素通しやマイグレーションの整合といった実績の核心が境界値の推論に依っているためです
