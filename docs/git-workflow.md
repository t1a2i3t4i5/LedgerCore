...
PR 本文は次の形で書きます。squash merge なので、**この本文がそのまま `main` のコミットメッセージ本体として残ります**。
...
| 節 | 消してよい条件 | 何を書くか |
| ---- | ---- | ---- |
| `Closes #` | 対応する issue が無いとき | issue の自動クローズ |
| やったこと | 消さない | 何をどう変えたかより「なぜその判断にしたか」。採らなかった案があるなら、採らなかった理由も残す。変更が複数あるなら `###` で分けて番号を振る |
| テスト | 消さない | 追加・変更したテストファイルと件数、何を確認しているか。テストを足さないなら「テストなし。理由は〜」と書く |
| テストが欠陥を捕まえることの確認 | ロジックを変えない PR(docs のみ、コメント修正など) | 実装を意図的に壊し、追加したテストが赤くなることを実測した結果 |
| 同梱物 | 該当する行が 1 つも無いとき | 生成物・docs の同梱漏れ。該当する行だけを残す(下記) |
| レビュー前に知っておいてほしいこと | 無いとき | 未検証の点、マージ前に必要な手順、特に見てほしい箇所 |

該当しない節は**節ごと消します**。とくに「テストが欠陥を捕まえることの確認」を節ごと消す形にしてあるのは、書かなかったことを意図的な判断として残すためです。テストを 10 件足したという申告だけでは、実装を素通しするテストが混ざっていても気づけません。

「同梱物」に書くのは条件と成果物のパスだけです。
...
理由と手順は [CLAUDE.md](../CLAUDE.md) 側にあります。ここに書き写すと、片方だけが更新されて陳腐化します。

固定のチェックリスト(`- [ ] flutter analyze 無警告` の並び)は置きません。毎回すべてチェック済みで並ぶだけになり、読む側の情報量がゼロになるためです。実行結果は「テスト」の 1 行で足ります。代わりに置くのは、該当する PR でだけ現れる条件付きの節です。常に出るものは目に入らなくなりますが、条件付きなら「該当するのに空」が異常として見えます。

##### `.github/pull_request_template.md` を置かない理由

GitHub の PR テンプレート機能は、この repo ではほぼ発火しません。

- **`gh` はローカルの `.github/` を読まず、GitHub API 経由でデフォルトブランチのテンプレートを取りに行きます**(`gh` 2.92.0 で実測)。作業ブランチに置いてコミットしても、その PR 自身には適用されません
- **`--body` / `--fill` を渡すとテンプレートは読まれません。** この repo の PR はほぼすべて Claude Code が `--body` 付きで作っています

型をこのドキュメントだけに置けば、情報源が 1 か所に収まり、`.github/` と docs のどちらかが陳腐化することもありません。
