...
| コマンド              | 用途                                       |
| --------------------- | ------------------------------------------ |
| `/review3`            | 観点を分けた 3 人の独立レビュアーで多角レビュー(正確性 / 設計・保守性 / テスト・ドキュメント) |
...
- **PR の既存コメントや issue はレビュアーに読ませません。** 過去の指摘が目に入ると、それをなぞるだけになって独立性が失われ、追加の発見が減ります。レビュアーには `gh pr view` 系を禁じており、`WebFetch` / `WebSearch` も渡していません
- **既に issue になっている指摘は「既知」に仕分けます。** 別 issue に切り出した指摘は PR 内のコードが変わらないので、レビューのたびに再掲されます。3 体の報告が出揃った**あと**に限り、集約側が open issue と突き合わせます(**PR のコメントは引き続き読みません** — 読むと 3 体の報告の答え合わせに使ってしまい、独立性が消えるためです)。**既知に仕分けた指摘は本編から外れ、次に挙げる実測検証も省かれます** — 報告では専用セクションに 1 行ずつ、未検証である旨を添えて残ります。**ただし「重大度が致命・重大のもの」「今回の差分が触っている*ファイル*のもの」「issue 本文に実測の裏付けが無いもの」は既知にしません** — 仕分けが欠陥を隠す側に回らないための除外条件です。行ではなくファイル単位で判定するのは、issue 本文の行番号が起票後のコミットでずれるためです。実測の裏付けを条件に入れているのは、未検証のまま切り出された指摘が既知経由で永久に未検証になる経路を塞ぐためで、その担保として **issue を作るときは本文の冒頭に `対象:` と `実測:` の 2 行を置く**決まりにしてあります。判定の詳細は `.claude/commands/review3.md` の「既に issue になっている指摘を仕分ける」節が唯一の情報源です
...
- **再現しなかった指摘は「却下」として報告に残します。** もっともらしいが誤った指摘は必ず混ざるので、黙って消さず理由とともに残し、同じ誤指摘の再発を防ぎます
- **3 人の報告が出揃ったことを、issue を読む前に成果物として固定します。** 集約側は統合した指摘の一覧(重大度・対象ファイル・検出人数)を先にユーザーへ提示してから issue を開きます。これがないと「報告はもう固定されている」が心構えの問題になり、issue を見たあとで指摘を後付けで削る余地が残ります
- **打ち切られたレビューを「指摘なし」と取り違えません。** 3 体はターン上限で強制終了すると出力フォーマットごと出ないため、報告の末尾に完了マーカーの 1 行を出すことを義務づけてあります。集約側はこの行が無い報告を「未レビュー」として扱い、その旨を報告に明記します
- **3 体を並列で回すぶん使用量は大きいので、消費の内訳を実測して抑えてあります。** 過去 8 回分を計測したところ、1 回のレビューにつき 3 体合計で総トークンが約 452 万、その内訳は cache 読み込み 404 万(**89.3%**)・cache 作成 42 万(9.2%)・出力 6.5 万(**1.4%**)でした。cache 読み込みは「1 ターンごとにそれまでの全コンテキストを読み直す」分なので、**効くのはターン数**です。そこで(1)各レビュアーの定義に探索の規律を持たせ、(2)`reviewer-design` と `reviewer-tests-docs` のモデルと `effort` を下げ、(3)3 体ともターン上限を下げました。**`reviewer-correctness` だけはモデルと `effort` を据え置いています** — `Infinity` の素通しやマイグレーションの整合といった実績の核心が境界値の推論に依っているためです(ターン上限は 3 体とも下げています)
  - **現在値は各エージェントの frontmatter が唯一の情報源です。** ここに実値を書き写すと、片方を変えた瞬間にもう片方が嘘になるので書きません。規律の中身も同じ理由で `.claude/agents/reviewer-*.md` の「探索のしかた」節に置いてあります
  - 実測の再現手順: `~/.claude/projects/<プロジェクト>/<セッション ID>/subagents/agent-*.jsonl` を読み、各行の `message.usage` を積み上げます。どのレビュアーの記録かは同じディレクトリの `.meta.json` の `agentType` で判別できます

課金が発生する `/code-review ultra` と違い、`/review3` は通常のサブエージェントで動くので Claude 側から起動できます。

> `.claude/agents/` を**新しく追加したとき**に加え、**既存の定義の `model` / `effort` / `maxTurns` を書き換えたとき**も Claude Code の再起動が必要です。セッション開始時に読み込まれた内容がそのまま使われるため、再起動しないと「設定を下げたつもりで旧設定のまま回っている」状態になり、効果の有無を誤読します。

##### 設定を変えたら検出力を確かめる

モデル・`effort`・`maxTurns`・探索の上限はいずれも**見落としを増やす方向**の設定です。しかし `/review3` はレビュアーも集約側も PR コメントを読まないため、**過去の検出実績と突き合わせる手段が構造的にありません**。検出力が半減しても「今回は指摘 0 件でした」と見分けがつきません。

そこで、設定を変えたら次の手順で既知の欠陥を仕込み、拾えるかを確かめます。

1. `lib/db/database.dart` の `amount` の CHECK 制約を `amount >= 0` に書き換える
2. `lib/screens/add_transaction_screen.dart` の validator の下限判定を `< 0` に書き換える
3. `/review3`(作業差分モード)を回す
4. **`Infinity` と 0 円が素通しになること**(正確性)と、**それを捕まえる回帰テストが無いこと**(テスト)の両方が挙がるかを見る
5. `git checkout -- lib/db/database.dart lib/screens/add_transaction_screen.dart` で戻し、`git status` がクリーンに戻ったことを確認する

両方挙がれば検出力は保たれています。片方しか挙がらない、または 0 件なら、下げた設定を戻して再確認します。**この手順自体が PR コメントに依存しない自己完結の基準**になります。
