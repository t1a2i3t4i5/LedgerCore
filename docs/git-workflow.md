...
引数なしで `gh pr create` すると、本文欄に `.github/pull_request_template.md` が読み込まれます。該当しない節は**節ごと消して**使います。とくに「テストが欠陥を捕まえることの確認」は、実装を意図的に壊して追加したテストが赤くなることを実測する節です。ロジックを変えない PR では消しますが、節ごと消す形にしてあるのは、書かなかったことを意図的な判断として残すためです。`<!-- -->` のコメントは GitHub 上では表示されません。ただし squash merge でコミットメッセージ本体になるので、消してから提出するのが望ましいです。

読み込みには 2 つ落とし穴があります。

- **`gh` はローカルのファイルではなく GitHub 上の `main` からテンプレートを取りに行きます。** 作業ブランチに置いてコミットしただけでは本文欄に出ません(`gh` 2.92.0 で実測)。テンプレートを編集した PR では、その PR 自身にテンプレートは適用されません
- **`--body` / `--fill` を渡すとテンプレートは読まれません。** Claude Code が作る PR も `--body` を渡すので、その場合は `.github/pull_request_template.md` を読んで手で節を揃えます

なお本文欄には、コミットメッセージから作られた既定値(コミットが 1 本ならその本文、複数なら件名の箇条書き)もあわせて入ります。テンプレートの節に書き直したら、既定値のほうは消して構いません。
