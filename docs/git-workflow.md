...
- **既に issue になっている指摘は「既知」に仕分けます。** 別 issue に切り出した指摘は PR 内のコードが変わらないので、レビューのたびに再掲されます。3 体の報告が出揃った**あと**に限り、集約側が open issue と突き合わせます(**PR のコメントは引き続き読みません** — 読むと 3 体の報告の答え合わせに使ってしまい、独立性が消えるためです)。除外条件を含む手順は `.claude/commands/review3.md` の 4-2 が唯一の情報源です
