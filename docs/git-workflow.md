...
- **3 体を並列で回すぶん使用量は大きいので、消費の内訳を実測して抑えてあります。** 過去 8 回分を計測したところ、1 回のレビューにつき 3 体合計で総トークンが約 452 万、その内訳は cache 読み込み 404 万(**89.3%**)・cache 作成 42 万(9.2%)・出力 6.5 万(**1.4%**)でした。cache 読み込みは「1 ターンごとにそれまでの全コンテキストを読み直す」分なので、**効くのはターン数**です。そこで(1)各レビュアーの定義に探索の規律(`Bash` は差分取得だけ・`Read` は範囲を絞る・`Read`＋`Grep` は 12 回まで)を持たせ、(2)`reviewer-design` と `reviewer-tests-docs` を `sonnet` + `effort: medium` に下げ、(3)**3 体とも `maxTurns` を 40 → 30** にしました。**`reviewer-correctness` のモデルと `effort` だけは `opus` + `high` のまま**です — `Infinity` の素通しやマイグレーションの整合といった実績の核心が境界値の推論に依っているためです(`maxTurns` は 3 体とも下げています)
