...
本文には `.github/pull_request_template.md` が自動で読み込まれます(引数なしの `gh pr create` と GitHub の Web UI の両方)。`--body` を渡した場合はテンプレートを上書きするので、その場合は自分で節を揃えてください。

該当しない節は**節ごと消して**使います。とくに「テストが欠陥を捕まえることの確認」は、実装を意図的に壊して追加テストが赤くなることを実測する節で、ロジックを変えない PR では消します。消す形にしてあるのは、書かなかったことを意図的な判断として残すためです。`<!-- -->` のコメントは GitHub 上では表示されませんが、squash merge でコミットメッセージ本体になるため、消してから提出するのが望ましいです。
