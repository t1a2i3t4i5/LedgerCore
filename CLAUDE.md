...
- 画面の大見出しは `widgets/page_header.dart` を使い、ルートタブでは本文と一緒にスクロールさせる。ホームは月・年で `MonthSelector`、全期間で `PageHeader` を使う。push 先のカテゴリ・メンバー管理は唯一の戻る導線を残す `PinnedBackPageHeader`、取引追加・編集画面の `AppBar` は #122 までの例外
