...
- カテゴリ・メンバーの識別色は `widgets/chart_palette.dart` を使い直書きしない。保存色があれば優先し、未設定時だけ ID 由来の色へフォールバックする。カテゴリの保存順は管理・取引入力・集計モデルで共有し、ホームの月次カテゴリ内訳だけ金額降順（同額は保存順）の上位4件にする。5件以上の全件表示シートは同じ並べ替え済みリストと月合計を受け取り、DB・Providerを参照しない
